...
function showApp() {
    ...
    // Activate treehouse screen
    const treehouseScreen = document.getElementById('treehouseScreen');
    if (treehouseScreen) {
        treehouseScreen.classList.add('active');
    }
    
    // Trigger Buddy3D initialization after a short delay to ensure DOM is ready
    setTimeout(() => {
        if (window.initBuddy3D) {
            window.initBuddy3D();
        }
    }, 100);
}
...
// Logic patterns - Expanded with 30+ patterns
const LogicPatterns = [
    // Color patterns
    ...
    // Star and moon patterns
    { pattern: ['⭐', '⭐', '🌙', '⭐', '⭐'], answer: '🌙', options: ['⭐', '🌙', '☀️'] },
    { pattern: ['🌙', '⭐', '🌙', '⭐'], answer: '🌙', options: ['⭐', '🌙', '☀️'] },
    { pattern: ['☀️', '🌙', '☀️', '🌙'], answer: '☀️', options: ['☀️', '🌙', '⭐'] },
    ...
    { pattern: ['3', '1', '3', '1'], answer: '3', options: ['1', '2', '3'] },
    { pattern: ['2', '2', '3', '2', '2'], answer: '3', options: ['1', '2', '3'] },
    ...
    // Weather patterns
    { pattern: ['☀️', '☁️', '☀️', '☁️'], answer: '☀️', options: ['☀️', '☁️', '🌧️'] },
    { pattern: ['🌧️', '☀️', '🌧️', '☀️'], answer: '🌧️', options: ['🌧️', '☀️', '☁️'] },
];
...
function speak(text) {
    if (!AppState.voiceEnabled || !window.speechSynthesis) {
        ...
        if (window.Buddy3D && window.Buddy3D.say) window.Buddy3D.say(text);
        return;
    }
    ...
    if (window.Buddy3D) {
        utterance.onstart = () => {
            if (window.Buddy3D.talk) window.Buddy3D.talk(true, text);
        };
        utterance.onend = () => {
            if (window.Buddy3D.talk) window.Buddy3D.talk(false);
        };
        utterance.onerror = () => {
            if (window.Buddy3D.talk) window.Buddy3D.talk(false);
        };
        utterance.onboundary = (evt) => {
            if (window.Buddy3D.onBoundary) window.Buddy3D.onBoundary(evt);
        };
    }
...
// Expose to window so it can be wrapped by Buddy3D initializer
...
function buddyReact(emotion) {
    ...
    if (window.Buddy3D) {
        if (window.Buddy3D.setEmotion) window.Buddy3D.setEmotion(emotion);
        if (emotion === 'happy' || emotion === 'excited') {
            if (window.Buddy3D.poke) window.Buddy3D.poke();
        }
    }
        ...
        card.addEventListener('click', (e) => {
            ...
            switch(activity) {
                ...
                case 'time':
                    ...
                    setTimeout(() => generateTimeChallenge(), 50); // Small delay to ensure DOM is ready
...
const TrackerState = {
    ...
    currentReward: null,
    activities: [
        // Sample tasks for demo - parents can add more
        { id: 'task1', title: 'Complete ABC Activity', description: 'Learn letters A-Z', icon: '📚', points: 10 },
        { id: 'task2', title: 'Practice Counting', description: 'Count to 20', icon: '🔢', points: 10 },
        { id: 'task3', title: 'Tell Time', description: 'Practice reading the clock', icon: '🕐', points: 10 },
        { id: 'task4', title: 'Rhyming Words', description: 'Find words that rhyme', icon: '🎵', points: 10 },
        { id: 'task5', title: 'Logic Puzzle', description: 'Complete pattern challenges', icon: '🧩', points: 10 }
    ],
    completions: [],
...
// ===== Buddy3D + BuddyVoice Boot =====
document.addEventListener('DOMContentLoaded', () => {
    // Mount 3D Buddy with retry if container isn't ready
    let initAttempts = 0;
    const maxAttempts = 50; // Max 5 seconds of retries
    
    function initBuddy3D() {
        if (window.Buddy3D && !window.buddy3dInstance) {
            const container = document.getElementById('buddy3d');
            const treehouseScreen = document.getElementById('treehouseScreen');
            
            if (container && treehouseScreen) {
                // Check if treehouse screen is visible
                const isTreehouseVisible = treehouseScreen.classList.contains('active');
                const rect = container.getBoundingClientRect();
                
                // Only initialize if container has dimensions OR treehouse is visible
                if ((rect.width > 0 && rect.height > 0) || isTreehouseVisible) {
                    console.log("Initializing Buddy3D with dimensions:", rect.width, "x", rect.height);
                    window.buddy3dInstance = window.Buddy3D.attach('buddy3d');
                    
                    if (window.buddy3dInstance) {
                        console.log("✅ Buddy3D initialized successfully!");
                        
                        // Add compatibility shims for old API
                        window.Buddy3D.talk = function(talking, text) {
                            if (!window.buddy3dInstance) return;
                            if (talking && text) {
                                window.buddy3dInstance.speak(text);
                            } else if (!talking) {
                                window.buddy3dInstance.stopSpeaking();
                            }
                        };
                        
                        window.Buddy3D.onBoundary = function(evt) {
                            // Boundary events handled internally by speak()
                        };
                        
                        window.Buddy3D.say = function(text) {
                            if (window.buddy3dInstance) {
                                window.buddy3dInstance.speak(text);
                            }
                        };
                        
                        window.Buddy3D.setEmotion = function(emotion) {
                            if (!window.buddy3dInstance) return;
                            // Map emotions to animations
                            if (emotion === 'happy' || emotion === 'excited') {
                                window.buddy3dInstance.setSmile(0.8);
                                window.buddy3dInstance.setBrow(0.3);
                            } else if (emotion === 'sad') {
                                window.buddy3dInstance.setSmile(0.1);
                                window.buddy3dInstance.setBrow(-0.5);
                            } else if (emotion === 'thinking') {
                                window.buddy3dInstance.startThinking();
                            } else {
                                window.buddy3dInstance.setSmile(0.35);
                                window.buddy3dInstance.setBrow(0);
                            }
                        };
                        
                        window.Buddy3D.poke = function() {
                            if (window.buddy3dInstance) {
                                window.buddy3dInstance.giggle();
                            }
                        };
                    }
                } else if (initAttempts < maxAttempts) {
                    initAttempts++;
                    console.log("Buddy3D container has zero dimensions, retrying in 100ms... (attempt " + initAttempts + "/" + maxAttempts + ")");
                    setTimeout(initBuddy3D, 100);
                } else {
                    console.log("⚠️ Buddy3D initialization skipped - container not visible. Will initialize when treehouse screen becomes active.");
                }
            }
        }
    }
    
    // Expose to window so showApp() can call it
    window.initBuddy3D = initBuddy3D;
    
    // Try immediately
    initBuddy3D();
    
    // Re-initialize when switching to treehouse screen
    const originalSwitchScreen = window.switchScreen;
    if (originalSwitchScreen) {
        window.switchScreen = function(screenId) {
            originalSwitchScreen(screenId);
            if (screenId === 'treehouseScreen' && !window.buddy3dInstance) {
    ...
    const waveCanvas = document.getElementById('micWaveform');
    const waveCtx  = waveCanvas ? waveCanvas.getContext('2d') : null;

    if (!micBtn || !window.BuddyVoice) return;

    // Toggle recording on mic button tap
    micBtn.addEventListener('click', () => {
        window.BuddyVoice.ensureAudioCtx(); // must happen in user gesture
        if (window.BuddyVoice.recording) {
            window.BuddyVoice.stopRecording();
        } else {
            window.BuddyVoice.startRecording().catch(err => {
                console.warn('Mic access denied', err);
                micLabel.textContent = 'Allow mic!';
            });
        }
    });
    ...
    if (jokeBtn) {
        jokeBtn.addEventListener('click', () => {
            window.BuddyVoice.ensureAudioCtx();
            jokeBtn.disabled = true;
            micBtn.disabled  = true;
            window.BuddyVoice.tellJoke();
        });
    }

    // ── BuddyVoice event listeners ──
    window.BuddyVoice.on('recordingStart', () => {
        micBtn.classList.add('recording');
        micBtn.classList.remove('playing');
        micLabel.textContent = 'Stop';
        if (jokeBtn) jokeBtn.disabled = true;
    });

    window.BuddyVoice.on('recordingStop', () => {
        micBtn.classList.remove('recording');
        micLabel.textContent = 'Processing…';
        micBtn.disabled = true;
    });

    window.BuddyVoice.on('processing', () => {
        micLabel.textContent = 'Thinking…';
    });

    window.BuddyVoice.on('playbackStart', () => {
        micBtn.classList.add('playing');
        micLabel.textContent = 'Buddy says…';
        micBtn.disabled = true;
        if (jokeBtn) jokeBtn.disabled = true;
    });

    window.BuddyVoice.on('playbackEnd', () => {
        micBtn.classList.remove('playing');
        micBtn.disabled = false;
        micLabel.textContent = 'Talk to me!';
        if (jokeBtn) jokeBtn.disabled = false;
    });

    window.BuddyVoice.on('tooShort', () => {
        micBtn.disabled = false;
        micLabel.textContent = 'Try again!';
        if (jokeBtn) jokeBtn.disabled = false;
        setTimeout(() => { micLabel.textContent = 'Talk to me!'; }, 1800);
    });

    window.BuddyVoice.on('jokeEnd', () => {
        micBtn.disabled  = false;
        if (jokeBtn) jokeBtn.disabled = false;
        micLabel.textContent = 'Talk to me!';
    });
    ...
    function drawWaveform() {
        if (!waveCtx || !waveCanvas) { requestAnimationFrame(drawWaveform); return; }
        const w = waveCanvas.width;
        const h = waveCanvas.height;
        waveCtx.clearRect(0, 0, w, h);

        const data = window.BuddyVoice.getWaveformData();
        if (data && data.length > 0) {
            const sliceWidth = w / data.length;
            // Warm amber waveform color
            waveCtx.strokeStyle = window.BuddyVoice.recording ? '#e05050' : '#c07a3a';
            waveCtx.lineWidth = 2;
