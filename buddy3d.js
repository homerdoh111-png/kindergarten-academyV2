...
(function () {
  ...
  function init(container) {
    ...
    if (!THREE.GLTFLoader) {
      ...
      // Load from GitHub raw URL (corrected repo name with capital V)
      const modelPath = 'https://raw.githubusercontent.com/homerdoh111-png/kindergarten-academyV2/main/buddy-model.glb';
    ...
    loader.load(
      ...
      function (gltf) {
        ...
        console.log('Buddy3D: Model info:', buddyModel);
        
        // Calculate bounding box to understand model size
        const box = new THREE.Box3().setFromObject(buddyModel);
        const size = box.getSize(new THREE.Vector3());
        const center = box.getCenter(new THREE.Vector3());
        console.log('Buddy3D: Model size:', size);
        console.log('Buddy3D: Model center:', center);
        
        // Scale the model to fit in view (adjust multiplier as needed)
        const desiredHeight = 2; // Target height in scene units
        const currentHeight = size.y;
        const scaleFactor = desiredHeight / currentHeight;
        
        buddyModel.scale.set(scaleFactor, scaleFactor, scaleFactor);
        
        // Center the model at origin and raise it to ground level
        buddyModel.position.set(-center.x * scaleFactor, -center.y * scaleFactor + 0.5, -center.z * scaleFactor);
        
        console.log('Buddy3D: Applied scale:', scaleFactor);
        console.log('Buddy3D: Final position:', buddyModel.position);
